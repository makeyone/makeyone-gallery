...
export const getComboKeyProps = (
  ...
  const getPolygonPath = (corners: number[][]) => `polygon(${corners.map((c) => `${100 * c[0]}% ${100 * c[1]}%`).join(',')})`;
...
export function calculatePointPosition({ x = 0, x2 = 0, y = 0, r = 0, rx = 0, ry = 0, w = 0, w2 = 0, h = 0 }: VIAKey) {
  ...
  const yPos = CSSVarObject.keyYPos * y + (h * CSSVarObject.keyHeight) / 2 + ((h - 1) * CSSVarObject.keyYSpacing) / 2;
...
const getRowForKey = (k: VIAKey, suggestedRow: number) => {
  ...
  return widthProfiles[k.w] ? (widthProfiles[k.w].includes(suggestedRow) ? suggestedRow : widthProfiles[k.w][0]) : 4;
};
...
export const getMeshName = (k: VIAKey, profile: number, isLastRow: boolean) => {
  ...
  if (k.w === 1.25 && k.w2 === 1.5) {
    return `K-R2-ISO`;
  }
  if (k.w === 1.5 && k.w2 === 2.25) {
    return `K-R2-BAE`;
  }
  if (k.h > 1) {
...
export const getLabel = (
  ...
  if (
    ...
    selectedDefinition.customKeycodes[getCustomKeycodeIndex(keycodeByte, basicKeyToByte)] !== undefined
  ) {
    ...
    tooltipLabel = getShortNameForKeycode(selectedDefinition.customKeycodes[customKeycodeIdx] as IKeycode, 700);
  } else if (keycodeByte) {
