...
export const getRandomColor = () =>
  Array(3)
    .fill(0)
    .reduce((a) => `${a}${(~~(Math.random() * 255)).toString(16).padStart(2, '0')}`, '#');

export function getRGBPrime(hue: number, c: number, x: number): [number, number, number] {
  if (hue >= 0 && hue < 60) {
    return [c, x, 0];
  }
  if (hue >= 60 && hue < 120) {
...
export const get256HSV = (color: string) => {
  ...
  return [Math.round((255 * h) / 360), Math.round(255 * s), Math.round(255 * v)];
};
export const getHSV = (color: string) => {
  const [rPrime, gPrime, bPrime] = getColorByte(color).map((c) => c / 255);
  const [cmax, cmin] = [Math.max(rPrime, gPrime, bPrime), Math.min(rPrime, gPrime, bPrime)];
