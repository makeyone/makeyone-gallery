...
const quantumRanges = (basicKeyToByte: Record<string, number>): Record<string, number> => {
  return Object.keys(basicKeyToByte).reduce(
    (acc, key) => (quantumRangesKeys.includes(key) ? { ...acc, [key]: basicKeyToByte[key] } : acc),
    {},
  );
};
...
const leftModifierValueToKey: Record<number, string> = Object.entries(modifierKeyToValue)
  .filter(([_, value]) => Object.values(modCodes).includes(value) && value < modCodes.QK_RMODS_MIN)
  .reduce((acc, [key, value]) => ({ ...acc, [value]: key }), {});

// Single right modifiers (as opposed to combos)
const rightModifierValueToKey: Record<number, string> = Object.entries(modifierKeyToValue)
  .filter(([_, value]) => Object.values(modCodes).includes(value) && value >= modCodes.QK_RMODS_MIN)
  .reduce((acc, [key, value]) => ({ ...acc, [value]: key }), {});

const topLevelValueToMacro = (basicKeyToByte: Record<string, number>): Record<number, string> => {
...
export const advancedStringToKeycode = (inputString: string, basicKeyToByte: Record<string, number>): number => {
  ...
  if (Object.keys(modifierKeyToValue).includes(parts[0])) {
...
export const advancedKeycodeToString = (
  ...
  for (let i = 0; i < valueToRange.length; i += 2) {
    const firstValueToRange = valueToRange[i][0];
    const nextValueToRange = valueToRange[i + 1][0];
    if (typeof valueToRange[i][0] === 'number' && typeof valueToRange[i + 1][0] === 'number') {
      if (firstValueToRange && nextValueToRange) {
  ...
  switch (lastRange) {
    ...
    case '_QK_ONE_SHOT_MOD':
      ...
      break;
    case '_QK_LAYER_MOD':
      const mask = basicKeyToByte._QK_LAYER_MOD_MASK;
      const shift = Math.log2(mask + 1);
...
const modValueToString = (modMask: number): string => {
  ...
  const qualifyingStrings = Object.entries(modMasks)
    .filter((part) => !excluded.includes(part[0]) && (part[1] & modMask) === part[1])
    .map((part) => part[0]);
  return qualifyingStrings.join(' | ');
};

const topLevelModToString = (keycode: number, byteToKey: Record<number, string>): string => {
  ...
  const enabledMods = Object.entries(modifierValue & modCodes.QK_RMODS_MIN ? rightModifierValueToKey : leftModifierValueToKey)
    .filter((part) => {
...
const parseTopLevelMacro = (inputParts: string[], basicKeyToByte: Record<string, number>): number => {
...
const parseMods = (input: string = ''): number => {
  ...
  return parts.reduce((acc, part) => acc | modMasks[part as keyof typeof modMasks], 0);
};

const parseModifierCode = (inputParts: string[], basicKeyToByte: any): number => {
  ...
  const bytes = realParts.map((part, idx) => {
    ...
    return modifierKeyToValue.hasOwnProperty(part) ? modifierKeyToValue[part as keyof typeof modifierKeyToValue] : null;
  });
...
export const anyKeycodeToString = (input: number, basicKeyToByte: Record<string, number>, byteToKey: Record<number, string>) => {
  let currentValue = '';
